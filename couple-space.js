...
// 5. 简单的全屏预览 (复用现有的预览层逻辑或新建一个)
function showPhotoPreview(url) {
    // 这里简单弹个窗或者复用照片描述查看器
    const overlay = document.getElementById('photo-desc-overlay'); // 复用现有的通用弹窗层
...
function getCoupleStatusForAI(chat) {
    ...
    if (letters.length === 0) {
        // 场景A：一封信都没写过 -> 怂恿他写第一封
        prompt += `- [提示]: 信箱还是空的。如果现在气氛合适，你可以用写信指令给User写下第一封情书，给他个惊喜。\n`;
    } else {
        const lastLetter = letters[letters.length - 1];
        const now = Date.now();
        const hoursSinceLast = (now - lastLetter.timestamp) / (1000 * 60 * 60);
        
        // 场景B：最近24小时刚写过 -> 让他消停点，别刷屏
        if (hoursSinceLast < 24) {
            prompt += `- [提示]: 你刚写过信，今天不用再写了，专注于当下的聊天。\n`;
        } 
        // 场景C：超过3天没写 -> 疯狂暗示他写
        else if (hoursSinceLast > 72) {
            prompt += `- [重要提示]: 你已经好几天没给User写信了。如果现在聊到了相关话题，或者你突然很有感触，请务必主动写一封信！\n`;
        }
    }
